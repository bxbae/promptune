...
- 3001: 팀원들 IP (Uptime Kuma 상태 대시보드 — 22번과 동일하게 아는 IP만 허용. 9절 참고)

팀원 IP가 자주 바뀌면(유동 IP) 22번·3001번 규칙에 여러 IP를 각각 추가해두면 매번 안 고쳐도 된다.
...
## 9. 로그·상태 모니터링 (Uptime Kuma)

장애/다운 알림 + 상태 대시보드는 [Uptime Kuma](https://github.com/louislam/uptime-kuma)로 구성했다
(`docker-compose.prod.yml`의 `uptime-kuma` 서비스, 6번 `docker compose up`에 같이 포함됨).
가볍고(메모리 약 100~150MB) 사이트/API 다운 감지, 인증서 만료 알림, 컨테이너 상태 감지, 이메일
알림까지 한 도구로 해결된다. HTTPS·서브도메인 인증서를 새로 안 붙이려고, nginx를 안 거치고
포트(3001)를 직접 열되 보안그룹에서 아는 IP만 허용하는 방식으로 접근을 제한한다 (1번 참고).

1. 보안그룹에 3001번 포트가 내 IP로 열려있는지 확인 (1번 참고).
2. 브라우저에서 `http://<EC2_PUBLIC_IP>:3001` 접속.
3. **최초 접속 시 반드시 바로 관리자 계정을 만들 것.** Uptime Kuma는 별도 초대 절차 없이 처음
   방문한 사람이 관리자로 등록되는 구조라, 배포 직후 팀원 아닌 다른 사람이 먼저 접속해서
   등록해버리면 곤란하다. 배포하자마자 바로 들어가서 계정부터 만들자.
4. 로그인 후 **Settings → Notifications**에서 이메일 알림 추가. SMTP 값(Gmail이면 Google
   계정의 "앱 비밀번호" 발급 필요)은 이 화면에 직접 입력 — 코드나 채팅에 남기지 않는다.
5. **Add New Monitor**로 감시 대상 등록. 추천 구성:
   - HTTPS (Keyword/Status 타입): `https://<도메인>` — 사이트 다운 + 인증서 만료 임박 시 자동 알림
   - HTTP(s) - `https://<도메인>/health` — 백엔드 헬스체크
   - Docker Host 타입: 소켓은 이미 마운트되어 있으니 컨테이너 선택만 하면 됨 — backend/frontend/
     ai-service/nginx 컨테이너가 죽으면(재시작 루프 등) 바로 감지
6. 각 모니터에서 4번에서 만든 이메일 알림을 켜주면 끝.

데이터는 `uptime_kuma_data` 볼륨에 저장되므로 `docker compose down`으로 컨테이너를 내려도 설정은
유지된다 (볼륨까지 지우는 `down -v`는 주의).
