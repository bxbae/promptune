...
#
# 2026-08-27: "리센느. 요약해줘. ... 3~4줄로, ... 숫자는 꼭 포함해서" 요청이
# 재시도까지 두 번 다 검증 실패해 503("검증을 통과하는 답변을 생성하지
# 못했습니다")으로 노출된 사례가 팀원 리포트로 재현됨. 원인은 이 정규식이
# "3~4줄로"처럼 범위(~)로 된 서식 지시어를 처리 못 해서 - "4줄"만 지워지고
# "3"은 그대로 남아 "반드시 결과에 있어야 하는 사실 숫자"로 오인됨(생성된
# 요약문에 우연히 숫자 "3"이 없으면 바로 실패). rule_validator.py(real
# 검증 경로)의 _FORMAT_DIRECTIVE_NUMBER_RE에는 이미 "(?:~\d+)?" range 처리가
# 있었는데, 이 mock 쪽 정규식(2026-08-25에 별도로 추가됨)에는 그 수정이
# 반영되지 않아 같은 클래스의 버그가 재발한 것 - 동일한 range 처리를
# 여기에도 추가한다.
_FORMAT_INSTRUCTION_NUM_RE = re.compile(
    r"\d+(?:~\d+)?\s*(?:문단|문장|줄|개|가지|번째|자|단어|페이지|포인트|배|위|점)"
)
