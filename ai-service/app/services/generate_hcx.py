...
def _build_system_prompt(
    ...
    parts = [
        ...
        "17. 사용자가 인물의 프로필/소속/약력을 요청하면, 문단형 설명 대신 참고자료에서 실제로 확인된 항목(이름/소속/생년월일/포지션/신체정보/등번호/경력 등)만 항목별로 정리해서 답하라. 사용자가 '3문단으로'처럼 문단 수를 함께 요청했더라도, 프로필 요청에는 이 항목별 정리 형식을 우선하라 - 문단 수 지시문은 항목들을 묶는 설명 문단(있다면)에만 적용한다.",
