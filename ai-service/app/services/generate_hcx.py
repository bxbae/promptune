...
def _build_system_prompt(
    ...
    parts.extend([
        ...
        "5. 사용자가 실존 공적 인물(운동선수·연예인·공직자 등)의 프로필/소속/약력을 조사해서 알려달라고 요청하면 (메일 수신자·문서 담당자·회의 참석자로 이름/직함이 언급된 경우는 여기 해당하지 않는다 - 그런 경우는 평소대로 업무 문서 작성 요청으로 처리하라), 문단형 설명 대신 다음 구조로 정리해서 답하라: '개요'(누구인지 1~2문장으로 정성적으로 요약 - 생년월일/출신지/신체정보 같은 개별 수치는 여기서 반복하지 말고 '기본 프로필'에서만 다뤄라) -> '기본 프로필'(이름/생년월일/출신지/신체정보/포지션 또는 직업/소속 등 항목별 나열) -> '경력'(과거~현재 소속/활동 이력을 시간순으로) -> '주요 특징'(참고자료에 실제로 있는 대표 기록·수상 등, 있는 경우만). 각 섹션은 참고자료에서 실제로 확인된 항목만 채우고, 확인되는 내용이 전혀 없는 섹션은 통째로 생략하라. 섹션 간에 같은 사실을 중복해서 다시 쓰지 마라. 사용자가 '3문단으로'처럼 문단 수를 함께 요청했더라도, 프로필 요청에는 이 구조를 우선하라 - 문단 수 지시문은 '개요' 같은 설명 문단에만 적용한다.",
