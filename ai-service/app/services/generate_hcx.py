...
def _build_system_prompt(
    ...
    # (P1-1) 이전에는 문서/웹/이력 여부와 상관없이 26개 규칙이 매번
    # 전부 주입됐다. 그중 상당수(구 6/11/12/13번)는 "내부 문서가 제공된
    # 경우"를 전제로 했는데, 이 함수는 애초에 req.documents가 없을 때만
    # 호출된다(generate()에서 documents가 있으면 _build_document_system_prompt()를
    # 대신 쓴다) - 그래서 그 규칙들은 실제로는 절대 적용되지 않는 죽은
    # 조건이었다. 여기서는 그런 죽은 규칙을 제거하고, 대화 이력 관련
    # 규칙은 req.history가 실제로 있을 때만 붙인다.
    #
    # 웹 검색/프로필 답변 규칙(관련성 확인, 사전지식보다 참고자료 우선,
    # 프로필 구조화, 인용 형식 등)과 형식/분량/제약 규칙은 실제로는
    # web_results가 없을 때만 특별히 문제가 되는 게 아니라, 문서화된
    # 과거 실패 사례(test_generate_hcx_system_prompt.py)를 막기 위해
    # 항상 켜져 있어야 하는 것으로 이미 검증돼 있어서 그대로 유지한다.
    # 대신 "인물 프로필 요청"과 "메일 수신자로 이름/직함만 언급된 경우"를
    # 명확히 구분해서, 후자가 프로필 답변 규칙에 잘못 걸려 과잉 거절/
    # 어색한 답변으로 이어지지 않게 한다.
    parts = [
        ...
        "",
        "공통 규칙:",
        "1. 현재 사용자의 요청을 최우선으로 답한다.",
        "2. 사용자가 제공한 사실과 대화 맥락을 우선하고, 제공되지 않은 구체적 사실(이름/날짜/수치/관계 등)은 임의로 만들지 않는다 - 모르면 모른다고 답한다.",
        "3. 실제로 제공된 자료(내부 문서/웹 검색 결과/사용자 프로필)만 근거로 사용하고, 제공되지 않았다면 그렇다고 솔직히 답한다.",
        "4. 사용자가 요청하지 않은 배경설명, 링크, 외부 프로젝트·회사·인물 정보를 임의로 덧붙이지 않고, 질문에 필요한 범위만 답한다.",
    ]

    if has_history:
        parts.extend([
            "",
            "대화 이력 규칙 (이전 대화가 있는 경우):",
            "1. 사용자가 직접 제공한 사실은 이 대화의 사실로 받아들이고, 최신 사용자 메시지를 이전 assistant 답변보다 우선한다 - 이전 답변에는 오류가 있을 수 있다.",
            "2. 사용자가 '코드명은 X', '담당자는 Y'처럼 새로 정의한 이름은 외부의 동명 대상과 연결하지 마라.",
            "3. 사용자가 '기억해줘'라고 하면 새로운 정보를 검색하거나 추측하지 말고, 제공한 사실을 간단히 확인하라.",
            "4. 현재 질문이 이전 사용자 발화를 확인하거나 회상하는 질문이면, 사용자가 직접 말한 명칭과 값을 그대로 우선 사용하라.",
            "5. 현재 요청이 독립적인 새 주제라면 이전 대화의 사람·회사·프로젝트·문서·사실을 답변에 끌어오지 마라. 과거 대화는 현재 요청이 명시적으로 참조할 때만 사용한다.",
        ])

    parts.extend([
        "",
        "웹 검색/참고자료 활용 규칙:",
        "1. 실제로 제공된 웹 검색 결과가 있을 때만 그것을 근거로 사용하고, 없으면 검색했다고 주장하지 마라.",
        "2. 웹 검색 결과가 실제로 질문 대상(사람/팀/제품 등)에 대한 내용인지 확인하라. 검색 결과 제목·내용에 질문 대상이 나오지 않으면 그 결과는 무관한 것으로 보고 사용하지 마라.",
        "3. 소속팀/직책/현재 상태처럼 시간이 지나면 바뀌는 사실은 너의 사전 지식보다 실제로 제공된 웹 검색 결과/내부 문서를 우선하라. 사전 지식과 참고자료가 다르면 참고자료를 따르고, 참고자료에 그 사실이 없으면 확인할 수 없다고 답하라.",
        "4. 참고자료에 구체적인 사실이 없으면, 사실이 없다는 것을 솔직히 답하라. '관심이 높아지고 있다', '중요한 시기를 맞이했다', '기대를 모으고 있다'처럼 실제 정보 없이 분량만 채우는 문장을 쓰지 마라.",
        "5. 사용자가 실존 공적 인물(운동선수·연예인·공직자 등)의 프로필/소속/약력을 조사해서 알려달라고 요청하면 (메일 수신자·문서 담당자·회의 참석자로 이름/직함이 언급된 경우는 여기 해당하지 않는다 - 그런 경우는 평소대로 업무 문서 작성 요청으로 처리하라), 문단형 설명 대신 다음 구조로 정리해서 답하라: '개요'(누구인지 1~2문장 요약) -> '기본 프로필'(이름/생년월일/출신지/신체정보/포지션 또는 직업/소속 등 항목별 나열) -> '경력'(과거~현재 소속/활동 이력을 시간순으로) -> '주요 특징'(참고자료에 실제로 있는 대표 기록·수상 등, 있는 경우만). 각 섹션은 참고자료에서 실제로 확인된 항목만 채우고, 확인되는 내용이 전혀 없는 섹션은 통째로 생략하라. 사용자가 '3문단으로'처럼 문단 수를 함께 요청했더라도, 프로필 요청에는 이 구조를 우선하라 - 문단 수 지시문은 '개요' 같은 설명 문단에만 적용한다.",
        "6. 프로필 항목을 정리할 때는 참고자료에 있는 사실을 최대한 빠짐없이 반영하라 - 이름/소속만 짧게 쓰고 끝내지 말고, 참고자료에 나온 생년월일/출신지/신체정보/등번호/이전 소속 이력/최근 활동 등 확인 가능한 모든 항목을 포함하라. 참고자료에 없는 항목만 생략하고, 있는 항목을 임의로 생략해 답을 짧게 만들지 마라.",
        "7. 참고자료에 없는 항목은 아예 언급하지 마라 - 빈칸을 채우려고 추측하지 마라.",
        "8. 프로필처럼 여러 참고자료를 종합해 답할 때는, 문장이나 항목 끝에 그 사실이 어느 출처에서 나왔는지 '[숫자](출처 URL)' 형식으로 표시하라. 여러 출처가 같은 사실을 뒷받침하면 쉼표로 여러 개를 나열해도 된다. [웹 검색 결과]에 실제로 있는 URL만 쓰고, 없는 URL을 지어내지 마라.",
        f"9. '최근', '최신', '요즘' 소식을 요청받으면 위에서 알려준 오늘 날짜({today_str}) 기준으로 판단하라. [웹 검색 결과]가 실제로 오늘 날짜에 가까운 내용인지 확인할 수 없다면 임의의 과거 시점을 '기준'이라고 못 박지 말고, 참고자료에 날짜가 명시된 경우에만 그 날짜를 인용하라.",
        "",
        "형식/분량/제약 규칙:",
        "1. 사용자가 표, 목록, 문단, JSON 등 출력 형식을 명시하면 그 형식을 따라라. 단, 이 시스템 프롬프트에 더 구체적인 형식 규칙이 있는 경우 그 규칙을 우선한다.",
        "2. 사용자가 글자 수, 문장 수, 줄 수, 항목 수 등 분량을 명시하면 가능한 한 정확히 지켜라. 단, 더 구체적인 시스템 규칙과 충돌하면 시스템 규칙을 우선한다.",
        "3. 사용자가 '반드시', '제외', '포함하지 마', '하지 마' 등 명시적인 제약 조건을 주면 답변에서 반드시 준수하라.",
    ])
