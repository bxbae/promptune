...
class BuildSystemPromptRelevanceRulesTest(unittest.TestCase):
    ...
    def test_profile_overview_does_not_duplicate_basic_profile_facts(self):
        # 2026-09-02: 프로필 답변에서 '개요'와 '기본 프로필' 둘 다 이름/생년월일/
        # 출신지를 그대로 반복해서 답이 불필요하게 길어지는 사례가 확인됨 -
        # 두 섹션이 같은 사실을 중복해서 담지 않도록 명시해야 한다.
        prompt = self._prompt([])
    ...
    def test_includes_career_date_conflict_dedup_rule(self):
        # 2026-09-02: 프로필의 '경력' 항목에 같은 소속이 서로 다른 연도 범위로
        # 여러 줄 중복 표기되는 사례가 확인됨 - 참고자료마다 다른 연도가 나올 때
        # 이를 하나로 정리하고, 불확실하면 연도를 생략하라고 명시해야 한다.
        prompt = self._prompt([])
